...
async fn test_store_and_retrieve_context() {
    ...
    // Store context
    let stored_context = context_service.store_context(content.to_string(), metadata).await
    ...
    let retrieved_context = context_service.get_context(stored_context.id).await
    ...
    assert_eq!(retrieved_context.metadata.tags, stored_context.metadata.tags);
    
    // List contexts with tags
    let contexts_with_tags = context_service.list_contexts(
        Some(vec!["test".to_string()]), 10, 0
    ).await.expect("Failed to list contexts");
    ...
    let updated_context = context_service.update_context(
        stored_context.id, updated_content.to_string(), updated_metadata
    ...
    context_service.delete_context(stored_context.id).await
