...
impl SimpleEmbeddingService {
    ...
    fn compute_embedding(&self, text: &str) -> Vec<f32> {
        ...
        for (word, count) in word_counts {
            ...
            let dimension = word.bytes().fold(0_usize, |acc, b| acc.wrapping_add(b as usize)) % self.embedding_dimension;
...
impl EmbeddingPort for SimpleEmbeddingService {
    ...
    async fn find_similar(&self, query: &str, limit: usize) -> McpResult<Vec<(ContextChunk, f32)>> {
        ...
        for (chunk_id, embedding) in &*embeddings {
            ...
            // Create a simplified chunk for demonstration
            let chunk = ContextChunk {
                chunk_id: *chunk_id,
                context_id: Uuid::nil(), // Placeholder
                content: format!("Simulated chunk content for {}", chunk_id),
    ...
    async fn find_similar_with_tags(&self, query: &str, _tags: &[String], limit: usize) -> McpResult<Vec<(ContextChunk, f32)>> {
